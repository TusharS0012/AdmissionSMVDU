...
async function finalAllotment() {
  ...
  try {
    ...
    while (!allocationComplete) {
      ...
      const newlyAllocatedGNGN = await allocateRound1GNGN(
        unallocatedStudents,
        round
      );
...
async function areAllApplicationsProcessed() {
  try {
    // Get the total number of students who have allocations
    const allocatedStudents = await prisma.allocatedSeat.findMany({
      select: {
        studentId: true,
      },
      distinct: ["studentId"], // Get distinct student IDs
    });

    const numberOfUniqueStudents = allocatedStudents.length;
...
async function exportAllocatedSeatsToCSV() {
  ...
  try {
    ...
    // Prepare CSV header
    const header =
      "StudentID,Name,Email,Phone,Category,Department Allocated,Allocation Round\n";

    // Prepare CSV rows
    const rows = students.flatMap((student) =>
      student.allocations.map(
        (allocation) =>
